...
const Suppliers = () => {
	...
	const { query } = useQuery(); // ✅ now available
	...
	// Sorting
	const [sortValue, setSortValue] = useState<SelectOption>(options[1]);
	...
	// ✅ If query exists, filter suppliers by shop name/contact/email
	...
	const shownSupplier = filteredSuppliers.filter(
		...
		<div style={outletStyles.container}>
			...
			<AnimatePresence>
				{viewSupplier && (
					...
					>
						<SupplierView
							onArchive={(supplier) => handleArchive(supplier)}
